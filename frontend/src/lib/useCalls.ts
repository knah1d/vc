...
export function useCalls() {
  ...
  function update(next: CallState | null) { callRef.current = next; setCall(next); }
  ...
  useEffect(() => {
    ...
    function answered({ callId, socketId }: { callId: string; socketId: string }) {
      if (callRef.current?.callId === callId && socketId !== socket.id) close("Call answered in another tab.", false);
    }
    function disconnected() { if (callRef.current || busy.current) close("Connection lost. Please try calling again once reconnected.", false); }
  ...
  async function start(conversationId: string, mode: CallMode) {
    ...
    try {
      ...
      if (attempt === revision.current) { busy.current = false; setPreparing(false); }
    }
  }
